# CMSPRO 规则技能包 · Qoder 规则入口

> 无 frontmatter 的 Qoder 规则随项目记忆始终生效。完整规则与文档以规则包（`{{RULES_ROOT}}/`）为单一来源：核心规则见 `{{RULES_ROOT}}/AGENTS.md`，详细规范见 `{{RULES_ROOT}}/rules/`，技能见 `{{RULES_ROOT}}/skills/`。本文件为核心精简版。
